### Hierachical Binary Logit regression in STAN
library(dplyr)
library(magrittr)
library(ggplot2)
library(reshape2)
...
hierarchical_binlogit_nocov="data {
...
set.seed(999999)
nobs=100
prior_gamma = data.frame(obs=1:100, gamma_11=rgamma(nobs/2, 1, 1), gamma_21=rgamma(nobs/2,2,1),
...
hbin_logit_Stan_fullcov <- stan(model_code = hierarchical_binlogit_fullcov,
                                            data = dat, chains = 3, iter = 4000, warmup = 2000,
...
summary(hbin_logit_Stan_fullcov_noncentered, pars=c("Omega"), probs = c(0.025, 0.975))
...
##########PLOTS#####################
pdens=mcmc_dens_overlay(posterior_fullcov_ncp, pars = c("delta[1]", "delta[2]", "delta[3]"))
pdens
...
p1=mcmc_trace(posterior_nocov, pars = c("Omega"),
